"use client"

import { motion } from 'framer-motion';
import ComputerCanvas from './canvas/ComputerCanvas';
import Link from 'next/link';

const Hero = () => {
  return (
    <section className='relative w-full h-screen mx-auto'>
      <div className='paddingX absolute inset-0 top-[120px] max-w-7xl mx-auto flex flex-row items-start gap-5'>
        <div className='flex flex-col justify-center items-center mt-5'>
          <div className='w-5 h-5 rounded-full bg-[#915eff]' />
          <div className='w-1 sm:h-80 h-40 violet-gradient' />
        </div>

        <div>
          <h1 className='heroHeadText text-white'>Hi, I'm &nbsp;
          <span className='text-[#915eff]'>Josias</span></h1>

          <p className='heroSubText mt-2 text-white-100'>
            I'm a Software Developer, content <br className='sm:block hidden' /> 
            creator and startup owner.
          </p>

        </div>
      </div>

      <ComputerCanvas />

      <div className='absolute xs:bottom-10 bottom-32
      w-full flex justify-center items-center'>
        <Link href='#about'>
          <div className='w-[35px] h-[64px] rounded-3xl border-4
          border-secundary flex justify-center items-start p-2'>
            <motion.div
              animate={{
                y: [0, 24, 0]
              }}
              transition={{
                duration: 1.5,
                repeat: Infinity,
                repeatType: 'loop'
              }}
              className='w-3 h-3 rounded-full bg-secondary mb-1'
            />
          </div>
        </Link>
      </div>
    </section>
  )
}

export default Hero